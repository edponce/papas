...
# MPI.Finalize() is called automatically at end
...
def parseArgs():
    ...
    if not os.path.isfile(args.netlogo_prog):
        if not os.path.islink(args.netlogo_prog):
            print('ERROR: NetLogo exec does not exists, ' + args.netlogo_prog)
            err += 1
    ...
    if args.setup_files_dir:
        if not os.path.isdir(args.setup_files_dir):
            print('ERROR: NetLogo setup files directory does not exists, ' +
                  args.setup_files_dir)
            err += 1
        else:
    ...
    if len(args.setup_files) > 0:
        for setup_file in args.setup_files:
            ...
            if not os.path.isfile(setup_file):
                print('ERROR: NetLogo setup file does not exists, ' +
                      setup_file)
                err += 1
    else:
